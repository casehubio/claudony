...
| [engine-cdi-exclude-types-sync.md](engine-cdi-exclude-types-sync.md) | quarkus.arc.exclude-types must stay in sync with CasehubEnabledProfile and CompletionTestProfile overrides | application.properties + engine-enabled test profiles |
...
| [vertx-execute-blocking-event-loop-thread-detection.md](vertx-execute-blocking-event-loop-thread-detection.md) | Use Context.isOnEventLoopThread() not isEventLoopContext() — executeBlocking workers inherit the event loop Context but are not on the event loop thread | ClaudonyReactiveCaseChannelProvider.listChannels(), JpaCaseLineageQuery.findCompletedWorkers(), ClaudonyReactiveWorkerContextProvider.buildContext() |
