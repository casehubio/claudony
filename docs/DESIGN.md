...
├── WorkerSessionMapping            — role↔session bridge (caseId:role→sessionId; role→sessionId fallback; sessionId→roleName reverse lookup)
...
**`WorkerSessionMapping`** — bridges CaseHub role names to Claudony tmux session UUIDs. Three maps: `caseId:role→sessionId` (precise), `role→sessionId` (fallback), and `sessionId→roleName` (reverse lookup for `InstanceActorIdProvider`). Re-registration cleans up the stale reverse entry. Limitation: concurrent same-role workers across cases (#93).
