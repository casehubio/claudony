...
**Baseline (as of 2026-08-01, after Epic B conversation maturity #177/#178/#190/#191/#192):** 16 in `claudony-core` + 175 in `claudony-casehub` + ~420 in `claudony-app` = **~611 total**. App module gained 8 tests: member join (ok, 404, idempotent), leave (ok, 404), auto-join on post, presence (ok, 404), channel creation (6 tests from #177). `ClaudonyCaseChannelProviderTest` updated for 4-channel `NormativeChannelLayout` (coordination channel added). `ChannelInitialisedObserverTest` updated: non-case channels now register the backend. Previous baseline: 596 (2026-07-26, after #184). Frontend: 25 vitest. E2E: 4 workbench tests. 1 known worktree-specific failure (`GitStatusTest` — git remote URL differs in worktrees). Docker required for dev/test (PostgreSQL via Dev Services).
