...
**Baseline (as of 2026-06-17, after CI SNAPSHOT realignment):** 6 in `claudony-core` + 172 in `claudony-casehub` + 408 in `claudony-app` = **586 total, 579 passing**. Known failing: 6 × `MeshResourceInterjectionTest` (Qhorus SNAPSHOT build 62: `updateLastActivity` JPQL positional-param issue causes HTTP 500 on all message dispatch paths — pre-existing, filed as qhorus issue); `ResearcherCaseCompletionTest` (engine SNAPSHOT: GoalReachedEventHandler→CaseStatusChangedHandler chain — tracked in #154). Previous baseline: 584 passing (2026-06-16 after #94 — that count was wrong; `MeshResourceInterjectionTest` was already failing 6/9 with the same SNAPSHOT). Engine SNAPSHOT changes absorbed: `CaseHubRuntime.signal()` now `CompletionStage<Void>`, `Worker.builder().function()` replaces lambda constructor, `QuartzRetryService` added to exclude-types.
